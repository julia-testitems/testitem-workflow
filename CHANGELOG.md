...
- **Coverage can now go to GitHub Code Quality instead of Codecov.** GitHub stores
  ...
  produce. A new `coverage-target` input (`codecov`, `github`, `both`, `none`) picks the
  destination, and `coverage-cobertura-path` says where the Cobertura report is written.
  It defaults to `codecov`, so a repository that does nothing keeps the behaviour it had.
  The destination is always explicit: the only endpoint that reports whether Code Quality
  is enabled refuses the Actions token with `403 Resource not accessible by integration`,
  even under `permissions: write-all`, so the workflow cannot detect it for you. Each
  matrix leg uploads under a label of its own, because GitHub keys a report by commit
  and label and legs sharing one would overwrite each other. A failed upload is an
  annotation, never a failed test leg. Fork pull requests and merge queue runs are
  skipped by the upload action itself. Requires `code-quality: write`, which the
  `permissions: write-all` in the documented caller covers. See the new **GitHub Code
  Quality coverage** section of the README.
