...
- `max-workers` (string, default `""`): Maximum number of parallel test processes. Empty leaves the default: the number of CPU threads, at most 8 and at most one per 3 GiB of system memory.
- `threads` (string, default `""`): Value for the test processes' `--threads`, for example `4`, `auto`, or `2,1`. Empty leaves Julia's default.
- `gc-between-testitems` (string, default `""`): `true`, `false`, or empty for the default (off). `true` runs a full garbage collection after every test item. This slows runs down, and on machines short of memory it slows them down a lot. It can help when test items hold memory outside the Julia heap that only finalizers release.
- `memory-threshold` (string, default `""`): Recycle a test process once its own resident memory exceeds this fraction, between 0 and 1, of total system memory. Off by default. Experimental.
